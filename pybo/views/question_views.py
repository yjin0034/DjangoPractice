...
def question_create(request):
    # POST 요청 방식
    # 질문 등록 화면에서 subject, content 항목에 값을 기입하고 '저장하기' 버튼을 누르면 이번에는 /pybo/question/create/ 페이지를 POST 방식으로 요청
    # (question_form.html의) form 태그에 action 속성을 지정하지 않아 현재 페이지가 디폴트 action으로 설정되기 때문
    if request.method == 'POST':
        # question_create 함수는 QuestionForm()을 사용
    ...
    # 질문 목록 화면에서 질문 '등록하기' 버튼을 클릭한 경우, /pybo/question/create/ 페이지가 GET 방식으로 요청되어 question_create 함수가 실행
        ...
        form = QuestionForm()
    context = {'form': form}  # {'form': form} : 템플릿에서 질문 등록 시 사용할 폼 엘리먼트를 생성할 때 쓰인다.
    return render(request, 'pybo/question_form.html', context)

# 질문 수정 관련 함수
@login_required(login_url='common:login')
def question_modify(request, question_id):
    # 전달받은 id에 해당하는 데이터 얻기
    question = get_object_or_404(Question, pk=question_id)
    # 로그인한 사용자와 수정하려는 질문의 글쓴이가 다를 경우에는, "수정권한이 없습니다"라는 오류를 발생
    ...
    # POST 요청 방식
    # 질문 수정 화면에서 "저장하기" 버튼을 클릭하면, http://localhost:8000/pybo/question/modify/2/ 페이지가 POST 방식으로 호출되어 데이터가 수정됨
    if request.method == "POST":
    ...
    # GET 요청 방식
    # 질문 상세 화면에서 "수정" 버튼을 클릭하면, http://localhost:8000/pybo/question/modify/2/ 페이지가 GET 방식으로 호출되어 질문 수정 화면이 보여짐
    else:
        # 질문 수정 화면에 조회된 질문의 제목과 내용이 반영될 수 있도록 다음과 같이 폼을 생성
        # 폼 생성시 이처럼 instance 값을 지정하면, 폼의 속성 값이 instance의 값으로 채워진다. 따라서 질문을 수정하는 화면에서 제목과 내용이 채워진 채로 보일 것.
        form = QuestionForm(instance=question)
    context = {'form': form}
    # 얻은 form 데이터(context)를 템플릿 파일(pybo/question_form.html)에 적용하여 HTML을 생성한 후 리턴
    return render(request, 'pybo/question_form.html', context)
